...
CORRECT_SUBTITLE_USER_QUERY = """<command>
请为我校正这份srt字幕
</command>
<movie_info>
{{
...
# 新的系统提示词 (翻译任务，同样移除敏感指令)
TRANSLATE_SUBTITLE_PROMPT = """你是一个多阶段、专家级的字幕分析与增强引擎。你的核心任务时接受一份日文SRT字幕，并输出一个经过精准修正和流畅翻译的最终版本。你必须严格遵循以下原则和工作流程。
...
4.  **格式化输出 (Formatted Output)**: 根据任务指令，以标准的SRT格式输出处理结果。
...
# 新的用户提示（用于翻译），增加了具体的风格指令，并将敏感部分放在这里
TRANSLATE_SUBTITLE_USER_QUERY = """<command>
请为我翻译这份srt字幕
</command>
<metadata>
这部影片的来源是一部日本成人电影
