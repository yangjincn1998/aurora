    ...
    def request(self, av_code: str, *args, **kwargs) -> str:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                raise requests.exceptions.HTTPError(f"番号 {av_code} 不存在 (404 Not Found)")
            ...
            if '404' in response.text and 'Not Found' in response.text:
                ...
                raise requests.exceptions.HTTPError(f"番号 {av_code} 不存在 (页面显示404)")

            response.raise_for_status()

            logger.info(f"成功获取番号 {av_code} 的HTML内容，长度: {len(response.text)} 字符")
                    ...
                    if genre_spans and len(genre_spans) > 1:  # 类别通常有多个
                        ...
                        for genre_span in genre_spans:
                            ...
                            if genre_link and not genre_span.find('button'):
                                ...
                                if category_name:
                                    categories.append(BilingualText(original=category_name))
                        if categories:
                            metadata.categories = categories
                            logger.debug(f"提取类别: {[c.original for c in categories]}")
                            break
